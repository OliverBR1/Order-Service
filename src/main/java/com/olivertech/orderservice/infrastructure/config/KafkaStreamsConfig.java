...
public class KafkaStreamsConfig {
    ...
    public KafkaStreamsConfiguration kafkaStreamsConfig(
        ...
        // Mensagem malformada: loga e continua em vez de parar a StreamThread.
        // Usa string literal pois a constante foi deprecada no Kafka 4.x.
        config.put("default.deserialization.exception.handler", LogAndContinueExceptionHandler.class);

        // EOS v2 padrão commita a cada 100ms (muito agressivo).
        // 1 segundo reduz pressão de rebalance sem impactar latência real.
        config.put(StreamsConfig.COMMIT_INTERVAL_MS_CONFIG, "1000");

        // Aumenta o session timeout para reduzir rebalances espúrios.
        // Padrão 10s é muito curto — qualquer GC pause já pode disparar rebalance.
        ...
        // Backoff exponencial para reconexão — evita flood de "Rebootstrapping" no log
        // Sem prefixo = aplica a TODOS os clientes internos: consumer, producer e AdminClient
        config.put("reconnect.backoff.ms",     "1000");
        config.put("reconnect.backoff.max.ms", "10000");
        config.put("connections.max.idle.ms",  "540000");
        // retry.backoff.ms: backoff entre tentativas de metadata/request — padrão 100ms.
        // Aumentar para 1000ms reduz o flood de "Rebootstrapping" durante graceful shutdown.
        config.put("retry.backoff.ms",         "1000");
        config.put("retry.backoff.max.ms",     "10000");
        // Prefixo consumer = adicionalmente garante que o consumer use o mesmo backoff
